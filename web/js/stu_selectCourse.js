...
function loadJxbxxZzxk(obj){
	...
	if(czzt=="0"){
		...
		$.extend(requestMap,{
			...
			"njdm_id":$("#njdm_id").val(),"bh_id":$("#bh_id").val(),"xbm":$("#xbm").val(),"xslbdm":$("#xslbdm").val(),"mzm":$("#mzm").val(),"xz":$("#xz").val(),"bbhzxjxb":$("#bbhzxjxb").val(),
			"ccdm":$("#ccdm").val(),"xsbj":$("#xsbj").val(),"sfkknj":$("#sfkknj").val(),"sfkkzy":$("#sfkkzy").val(),"kzybkxy":$("#kzybkxy").val(),
    ...
    if(isEnd=="true"){
		...
		if(parseInt(jspage)==step && $(".kc_head").length < step){//未瀑布加载前已到最后页时，就不需要再次访问数据库
			...
			$.extend(requestMap,{
				...
				"zyh_id":$("#zyh_id").val(),"zyfx_id":$("#zyfx_id").val(),"njdm_id":$("#njdm_id").val(),"bh_id":$("#bh_id").val(),
					...
					for(var i=0; i<data.length; i++){
						...
						if(l_kchid != mdA.kch_id){
							...
							if($("#zzxkgjcxkg_kcz").val()=="1"){
								s_html.push(" - <i id='xf_"+l_kchid+"'>"+mdA.xf+"</i> "+$.i18n.get("msg_xf")+"</span><span>"+$.i18n.get("msg_jxbgs")+"：<font class='jxbgsxx'>0</font></span><span id='zt_txt_"+mdA.kch_id+"'>"+$.i18n.get("msg_zt")+"："+$.i18n.get("txt-wx")+"</span><span id='kcz_txt_"+mdA.kch_id+"'>"+$.i18n.get("msg_sskcz")+":"+mdA.kzmc+"</span></h3>");
							}else{
