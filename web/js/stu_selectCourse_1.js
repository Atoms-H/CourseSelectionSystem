    ...
    if(jxbzls == 1 && sfqzxk != "1"){
        if($("#kxqxktskg").val()=="1" && $("#sfkxq").val()=="1" && $("#xqh_id").val()!=trObj.find(".xqh_id").text()){
            ...
            $.confirm($.i18n.get("msg_qdyxkxqkc"),function(isBoolean1){
                if(isBoolean1){
                    $.closeModal("confirmModal");
                    saveCourseCt(trObj,jxb_id,do_jxb_id,kch_id,jxbzls,'0');//直接保存选中课程
                }else{
                    $("#btn-xk-"+$.convertID(jxb_id)).removeAttr("disabled");
                }
            });
        ...
        if($("#kxqxktskg").val()=="1" && $("#sfkxq").val()=="1" && $("#xqh_id").val()!=trObj.find(".xqh_id").text()){
            ...
            $.confirm($.i18n.get("msg_qdyxkxqkc"),function(isBoolean){
                if(isBoolean){
                    $.closeModal("confirmModal");
                    saveQzCourse(trObj,kch_id,jxb_id,do_jxb_id,jxbzls,syqz);
                }else{
                    $("#btn-xk-"+$.convertID(jxb_id)).removeAttr("disabled");
                }
            });
        ...
        buttons:{
            success : {
                ...
                callback : function() {
                    if($("#ajaxForm").isValid()){
                        var qz = jQuery("#qz").val();
                        saveCourseCt(trObj,jxb_id,do_jxb_id,kch_id,jxbzls,qz);//直接保存选中课程
                    }else{
                        return false;
                    }
                }
...
function saveCourseCt(trObj,jxb_id,do_jxb_id,kch_id,jxbzls,qz){
    if($("#sfyxsksjct").val()=="1"){
        $.ajaxSetup({async:false});
        $.post(_path+"/xsxk/zzxkyzb_cxCtKcZyZzxkYzb.html",{
            jxb_ids:do_jxb_id,xkxnm:$("#xkxnm").val(),xkxqm:$("#xkxqm").val(),kch_id:kch_id
        },function(data){
            if(data.flag=="0"){
                $.error($.i18n.get("msg_wzyclxgly"));//出现未知异常，请与管理员联系
                $("#btn-xk-"+$.convertID(jxb_id)).removeAttr("disabled");
                ...
                saveCourse(trObj,jxb_id,do_jxb_id,kch_id,jxbzls,qz);
            }else{
            ...
            success : {
                ...
                callback : function() {
                    if($("#ajaxForm").isValid()){
                        ...
                        /*$("input[name='select_do_jxb']").each(function(index,item){//根据select_jxb隐藏域的值是否为空来判定各类教学班是否全部选定
                            do_jxb_arr.push($(this).val());
                        });*/
                        //var fjxb_id = $("#select_jxb_00").val();
                        saveDjxbCourseCt(trObj,kch_id,jxb_id,do_jxb_id,jxbzls,jxb_arr.join(","));
                    }
                    return false;
                }
            },
...
function saveDjxbCourseCt(trObj,kch_id,jxb_id,do_jxb_id,jxbzls,jxb_arr){
    if($("#sfyxsksjct").val()=="1"){
        $.ajaxSetup({async:false});
        $.post(_path+"/xsxk/zzxkyzb_cxCtKcZyZzxkYzb.html",{
            jxb_ids:jxb_arr,xkxnm:$("#xkxnm").val(),xkxqm:$("#xkxqm").val(),kch_id:kch_id
        },function(data){
            if(data.flag=="0"){
                $.error($.i18n.get("msg_wzyclxgly"));//出现未知异常，请与管理员联系
                return false;
            }else if(data.flag=="1"){
                saveDjxbCourseBc(trObj,kch_id,jxb_id,do_jxb_id,jxbzls,jxb_arr);
            }else{
                //当前教学班与教学班？？上课时间冲突，确定要选吗？
                $.confirm($.i18n.prop("msg_yqtjxbct", [data.msg]),function(isBoolean){
                    if(isBoolean){
...
function saveDjxbCourseBc(trObj,kch_id,jxb_id,do_jxb_id,jxbzls,jxb_arr){
    var tmp_html = "<ul>";
    ...
    $.post(_path+"/xsxk/zzxkyzbjk_xkBcZyZzxkYzb.html",{
        ...
        njdm_id:$("#njdm_id").val(),zyh_id:$("#zyh_id").val(),xklc:$("#xklc").val(),xkxnm:$("#xkxnm").val(),xkxqm:$("#xkxqm").val(),qz:$("#qz").val()
    },function(data){
        setTimeout(function(){
            if(data!=null){
                var flag = data.flag;
...
function refreshDataAddZzxk(trObj,jxb_id,do_jxb_id,kch_id,jxbzls,sxbj,qz){
    ...
    if(len==0){
        ...
        add_html.push(">"+$.i18n.get("msg_px")+"</td><td class='h_sxbj'>"+$.i18n.get("msg_xsf")+"</td><td class='h_jxb'>"+$.i18n.get("msg_jxbmc")+"</td><td class='h_teacher'>"+$.i18n.get("msg_jshzc")+"</td><td class='h_time'>"+$.i18n.get("msg_sksj")+"</td><td class='h_addr'>"+$.i18n.get("msg_jxdd")+"</td>");
        if($("#sfxskssj").val()=="1"){
